    ...
    <div class="flex justify-center items-center mt-3">
        <div class="w-full sm:max-w-lg">
            ...
            <div class="p-6 bg-white border-b border-gray-200 mt-3">
                ...
                <form method="POST" action="{{ route('remboursements.store', ['projet_id' => $projet_id]) }}">
                    @csrf
                    <div>
                            <!-- Montant du remboursement -->
                        <x-input-label class="mt-2" for="montant" :value="__('Montant du remboursement')" />
                        <div class="input-group">
                            <x-text-input id="montant" class="block mt-1 w-full form-control" type="number" name="montant" :value="old('montant')" step="0.1" required autofocus min="1" max="{{ $remboursement_restant }}"/>
                        ...
                        <select name="projet_id" class="form-select" required @if(isset($projet_id))  @endif>
                            @if(isset($projet_id))
                                @foreach($projets as $projet)
                                    @if ($projet_id == $projet->id)
                        ...
                        <input type="date" name="dateRemboursement" class="form-control" required min="{{ \Carbon\Carbon::parse($projet->dateDebut)->format('Y-m-d') }}">
