...
<!-- 
                ...
                <div class="w-full max-w-3xl mx-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700" style="min-height: 480px;">
                    ...
                    <div id="fullWidthTabContent" class="border-t border-gray-200 dark:border-gray-600">
                        ...
                        <!-- Onglet Versement -->
                        <div class="hidden px-4 pt-1 pb-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="about" role="tabpanel" aria-labelledby="about-tab">
                            @if ($projet->nom_status == 'Remboursé' && $remboursements->count() == 0)
                                    <p>Remboursement total </p>
                                @elseif ($projet->nom_status !== 'Remboursé' && $remboursements->count() == 0)
                                    <p>Aucun remboursement pour le moment</p>
                            ...
                            @foreach ($remboursements as $remboursement)
                                @php
                                    $counter++;
                                @endphp
                                <div class="flex items-center w-full my-3 px-6" >
                                    <p class="text-lg font-semibold mr-3 mb-0">{{ $counter }}.</p>
                                    <div class="flex items-center justify-arround w-full px-4 py-3 bg-green-100 rounded-xl shadow-sm">
                                        <!-- <span class="flex items-center justify-center w-6 h-6 bg-white rounded-full lg:h-8 lg:w-8 dark:bg-blue-800 shrink-0 my-0">
                                            <svg class="w-3.5 h-3.5 text-blue-600 lg:w-3 lg:h-3  
                                             dark:text-blue-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
                                            </svg>
                                        </span> -->
                                        <p class="font-semibold mb-0 text-left">{{ $remboursement->montant }}€</p>
                                        <p class="text-gray-600 ml-auto mb-0 text-right">{{ Carbon\Carbon::parse($remboursement->dateRemboursement)->format('d-m-Y') }}</p>

                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <!-- Fin onglet versement -->
